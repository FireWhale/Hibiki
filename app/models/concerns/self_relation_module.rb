module SelfRelationModule
  ...
  included do
    ...
    #Scope
      ...
      scope :without_self_relation_categories, ->(categories){joins("LEFT OUTER JOIN (#{"Related#{model_string.capitalize}s".constantize.where("related_#{model_string}s.category IN (?)", categories).to_sql}) t1 ON t1.#{model_string}1_id = #{model_string}s.id").where(:t1 => {:id => nil})}
      scope :filters_by_self_relation_categories, ->(include_categories, exclude_categories) {from("((#{self.with_self_relation_categories(include_categories).to_sql}) union (#{self.without_self_relation_categories(exclude_categories).to_sql})) #{self.table_name} ")}
