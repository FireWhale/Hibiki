    ...
    describe "JSON Tests" do
      ...
      context "with watchlist/collection options" do
        if described_class == User
          it "currently returns nothing with watchlists option" do
            expect(record.to_json(:watchlists => true)).to match_json_schema(:user)
          end

          it "currently returns nothing with collections option" do
            expect(record.to_json(:watchlists => true)).to match_json_schema(:user)
          end
        else
