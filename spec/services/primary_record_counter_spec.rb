...
describe PrimaryRecordCounter do
  ...
  describe 'Collection Counting' do
    ...
    it 'counts albums and songs in collection' do
      create_list(:collection, 3, :with_song, user: user, relationship: 'Collected')
      create_list(:collection, 2, :with_album, user: user, relationship: 'Collected')
      expect(PrimaryRecordCounter.perform('collection', user: user, collection_category: 'Collected')).to eq (5)
    end
    ...
    it 'counts albums and songs of a specific category in a collection' do
      ...
      create_list(:collection, 2, :with_album, user: user, relationship: 'Collected')
      expect(PrimaryRecordCounter.perform('collection', user: user, collection_category: 'Collected')).to eq (2)
    end
